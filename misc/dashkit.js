    ...
    var chart = function(div) {
      ...
      */
      ...
      if (scale == 'linear'){
        ...
        if (chart == 'bar'){
          x.rangeRoundBands([padding, width - padding], .5);
        }
        else if (chart == 'line'){
          x.rangePoints([padding, width - padding], 1); // second param controls tick pos
        }
      ...
      if(yAxis.minVal === undefined){
        y.domain([y_data_min, y_data_max]);
      }
      else {
        y.domain([yAxis.minVal, y_data_max]);
      }
      
      
      var tick_vals = svg.append("g")
          .attr("class", "x axis")
